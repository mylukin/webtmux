...
if (elem !== null) {
    ...
    // Debounced connection check to prevent rapid firing when multiple events trigger
    let checkPending = false;
    let checkTimer: ReturnType<typeof setTimeout> | null = null;

    const debouncedCheck = (source: string, delayMs: number) => {
        console.log(`[Main] ${source}, scheduling connection check...`);
        ...
        // If already pending, just update the timer
        if (checkPending) {
            checkTimer = setTimeout(() => {
                checkPending = false;
                checkTimer = null;
                console.log(`[Main] Executing debounced connection check (source: ${source})...`);
                wt.checkConnection();
            }, delayMs);
            return;
        }
    ...
    document.addEventListener("visibilitychange", () => {
        if (document.visibilityState === "visible") {
            debouncedCheck("Page visible", 100);
        }
    });

    // Secondary: Check connection when network comes back online
    window.addEventListener("online", () => {
        debouncedCheck("Network online", 500);
    });

    // Tertiary: Handle bfcache (back-forward cache) restoration
    window.addEventListener("pageshow", (event) => {
        if (event.persisted) {
            debouncedCheck("Page restored from bfcache", 100);
        }
    });

    // Additional: Handle focus event as backup for visibility change
    // Some browsers (especially Safari) may not fire visibilitychange reliably
    window.addEventListener("focus", () => {
        debouncedCheck("Window focus", 200);
    });
}
