...
UNINSTALL=false
AUTH_IP_BINDING=false # Default to proxy-friendly mode
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --non-interactive)
            ...
            ;;
        --auth-ip-binding)
            AUTH_IP_BINDING=true
            shift
            ;;
        --no-auth-ip-binding)
            AUTH_IP_BINDING=false
            shift
            ;;
        --port)
            LISTEN_PORT="$2"
        ...
        --help)
            ...
            echo "  --auth-ip-binding  Bind auth tokens to client IP (default: disabled)"
            echo "  --no-auth-ip-binding Disable auth token IP binding"
...
install_systemd_service() {
    ...
    # Enable fast reconnection (1 second)
    exec_cmd="${exec_cmd} --reconnect --reconnect-time 1"
    exec_cmd="${exec_cmd} --auth-ip-binding=${AUTH_IP_BINDING}"
...
install_launchd_service() {
    ...
    local auth_ip_arg="        <string>--auth-ip-binding=${AUTH_IP_BINDING}</string>"
...
main() {
    ...
    if [[ "$AUTH_IP_BINDING" == true ]]; then
        echo -e "  Auth IP binding: ${GREEN}ENABLED${NC}"
    else
        echo -e "  Auth IP binding: ${YELLOW}DISABLED${NC}"
    fi
    echo ""
    ...
    quick_cmd="${quick_cmd} --reconnect --reconnect-time 1"
    quick_cmd="${quick_cmd} --auth-ip-binding=${AUTH_IP_BINDING}"
